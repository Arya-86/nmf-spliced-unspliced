...
#
# Factor similarity and bipartite matching across modalities.
# ============================================================
...
# Extract and align feature spaces
concat_s_nmf  <- get_concat_block(sus_nmf, "spliced")
concat_us_nmf <- get_concat_block(sus_nmf, "unspliced")
...
# Use the unspliced block for concat-unspliced comparisons
G_adj <- RcppML::cosine(
  concat_us_nmf,
  us_nmf
)
...
# Reorder factors within each modality
order <- c(
  hclust(
    dist(t(G[, which(modalities == "concatenated")]))
  )$order,

  hclust(
    dist(t(G[, which(modalities == "added")]))
  )$order +
    min(which(modalities == "added")) - 1,

  hclust(
    dist(t(G[, which(modalities == "spliced")]))
  )$order +
    min(which(modalities == "spliced")) - 1,

  hclust(
    dist(t(G[, which(modalities == "unspliced")]))
  )$order +
...
for (i in 1:length(models_to_compare)) {
  ...
  for (j in 1:length(models_to_compare)) {

    if (i != j) {

      model1_name <- names(models_to_compare)[[i]]
      model2_name <- names(models_to_compare)[[j]]
      ...
      matched_costs <-
        get_bipartite_match_costs(cost)
...
# Panel B: bipartite matching cost
p2 <- ggplot(
  results,
  aes(
    model1,
...
factor_similarity_plot <- cowplot::plot_grid(
  p1,
  p2,
  nrow = 1,
  rel_widths = c(1, 0.7),
