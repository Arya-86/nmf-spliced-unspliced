...
#
# Factor similarity and bipartite matching for Figure 3.
# ============================================================
...
sus_nmf <- heart_concat_nmf$w
s_plus_us_nmf <- heart_added_nmf$w
s_nmf <- heart_spliced_nmf$w
us_nmf <- heart_unspliced_nmf$w
...
# Extract concatenated feature blocks
...
# L2-normalize factor loadings
w <- l2_normalize(w)

G <- crossprod(w)


# Concat-unspliced comparisons use the
# unspliced block of the concatenated model
G_adj <- RcppML::cosine(
  concat_us_nmf,
  us_nmf
)
...
# Reorder factors within modality
order <- c(
